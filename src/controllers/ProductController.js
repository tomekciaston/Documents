...
class ProductController {
  ...
  async indexRestaurant (req, res) {
    try {
      const products = await this.productService.indexRestaurant(req.params.restaurantId)
      res.json(products)
    } catch (err) {
  ...
  async update (req, res) {
    try {
      const updatedProduct = await this.productService.update(req.params.productId, req.body)
  ...
  async destroy (req, res) {
    try {
      ...
      const message = result ? 'Successfully deleted.' : 'Could not delete product.'
      res.json(message)
    } catch (err) {
      res.status(500).send(err.message)
    }
